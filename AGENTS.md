...
| **RU:** Параметр | **RU:** Возможные значения и действия | **EN:** Parameter | **EN:** Possible values and actions |
| --- | --- | --- | --- |
| `sandbox_mode` | **RU:** `read-only`, `workspace-write`, `danger-full-access`. Для операций за пределами `{{WRITABLE_DIRECTORIES}}` заранее запроси эскалацию. | `sandbox_mode` | **EN:** `read-only`, `workspace-write`, `danger-full-access`. Request escalation before acting outside `{{WRITABLE_DIRECTORIES}}`. |
| `network_access` | **RU:** `restricted` (нужна эскалация) или `enabled`. Любые внешние запросы, загрузки или установки согласовывай при режиме `restricted`. | `network_access` | **EN:** `restricted` (requires escalation) or `enabled`. Coordinate outbound requests, downloads, or installs when set to `restricted`. |
| `approval_policy` | **RU:** `never`, `on-request`, `on-failure`, `untrusted`. Всегда уточняй, какие команды требуют подтверждения, и сохраняй договорённость в `{{CHAT_CONTEXT_FILE}}`. | `approval_policy` | **EN:** `never`, `on-request`, `on-failure`, `untrusted`. Record which commands need approval in `{{CHAT_CONTEXT_FILE}}`. |
| Эскалация / Escalation | **RU:** Формируй запрос одно предложение + список команд (одна строка — одна команда) с ожидаемым результатом; используй канал `{{ESCALATION_CHANNEL}}`. | Escalation | **EN:** Provide a one-sentence justification plus a command list (one line per command) with expected outcomes; use `{{ESCALATION_CHANNEL}}`. |
| Проверки / Checks | **RU:** При `with_escalated_permissions=true` заранее опиши цель, риск и план отката; после выполнения зафиксируй результат в `{{SESSION_HISTORY_FILE}}`. | Checks | **EN:** Before setting `with_escalated_permissions=true`, state intent, risk, and rollback plan; log the outcome in `{{SESSION_HISTORY_FILE}}`. |

#### Матрица поведения / Behavior matrix
| `sandbox_mode` | `approval_policy` | **RU:** Разрешено / запрещено | **EN:** Allowed / disallowed |
| --- | --- | --- | --- |
| read-only | untrusted/never | **RU:** Только чтение, без записи и без запуска тестов, требующих файлового вывода. | **EN:** Read-only operations; no writes, no tests needing file output. |
| workspace-write | on-request/on-failure | **RU:** Разрешены безопасные записи в рабочую директорию; перед длительными проверками или установками пакетов запрашивай подтверждение. | **EN:** Safe writes within the workspace allowed; seek approval before long-running checks or package installs. |
| danger-full-access | on-request/never | **RU:** Любые действия согласуются индивидуально; всегда документируй шаги и результаты. | **EN:** Every action requires explicit agreement; document steps and outcomes thoroughly. |
...
- **RU:** Скрипты `scripts/consult.py`, `scripts/trim_consult_logs.py` и вспомогательные каталоги `tmp/consultation_runs/`, `tmp/assistant_contexts/` входят в шаблон по умолчанию. Используйте их для параллельных консультаций (`execute`) и пост-обработки (`process`), чтобы выполнять требования многоагентного протокола.
  **EN:** Ship `scripts/consult.py`, `scripts/trim_consult_logs.py`, and the helper folders `tmp/consultation_runs/`, `tmp/assistant_contexts/` with the template: run `execute` for parallel consultations and `process` for post-processing to satisfy the multi-assistant protocol.
...
- **RU:** Для пакетных консультаций используйте `scripts/consult.py execute -a gemini,qwen -p "<вопрос>"`, затем `scripts/consult.py process <run_id>`; по возможности подключайте `scripts/trim_consult_logs.py`, чтобы логи не разрастались.
  **EN:** For batch consultations run `scripts/consult.py execute -a gemini,qwen -p "<question>"` followed by `scripts/consult.py process <run_id>`; when available, use `scripts/trim_consult_logs.py` to keep transcripts compact.
...
- **RU:** Сверяй `AGENTS.md`, `README*.md`, `CONTRIBUTING*.md`, `README_snippet.md`, `scripts/*` с эталоном `{{UPSTREAM_AGENTS_URL}}` с частотой `{{SYNC_FREQUENCY}}` или при значимых изменениях.
  **EN:** Compare `AGENTS.md`, `README*.md`, `CONTRIBUTING*.md`, `README_snippet.md`, `scripts/*` with the canonical `{{UPSTREAM_AGENTS_URL}}` every `{{SYNC_FREQUENCY}}` or after major updates.
