...
- **RU:** Дополнения распространяются на всех локальных ассистентов (`gemini`, `qwen`, `codex`, `copilot`, `claude`) и описывают долгосрочные договорённости: окружения, проверки, модели.
- **EN:** These addenda apply to every local assistant (`gemini`, `qwen`, `codex`, `copilot`, `claude`) and capture long-lived agreements: environments, checks, and model choices.
...
- **RU:** Матрица окружений (добавьте строки по мере необходимости):
  **EN:** Environment matrix (extend with additional rows as needed):
  ...
  | --- | --- | --- | --- | --- |
  | `<OS / shell>` | `<что делаем>` | `<bash/pwsh/git/...>` | `<когда разрешено запускать gemini/qwen/codex/copilot/claude>` | `<что требует согласования, где писать>` |
...
- **RU:** Симлинки `.github/copilot-instructions.md`, `.claude/CLAUDE.md`, `.gemini/GEMINI.md`, `.qwen/QWEN.md`, `CLAUDE.md`, `GEMINI.md`, `QWEN.md` всегда относительные; README RU/EN синхронизируем при изменениях.
  **EN:** Keep `.github/copilot-instructions.md`, `.claude/CLAUDE.md`, `.gemini/GEMINI.md`, `.qwen/QWEN.md`, `CLAUDE.md`, `GEMINI.md`, `QWEN.md` as relative symlinks; keep RU/EN READMEs in sync.
...
- **RU:** После консультаций (`codex exec --json`, `gemini`, `qwen`, `copilot`, `claude`) прогоняем логи через `<consult_trim_script>`, складываем результаты в `tmp/ai/assistant_contexts/logs/<cli>/` и вложения в `tmp/ai/assistant_contexts/attachments/<cli>/`. Ссылки добавляем в ответы и `local/ai/session_history.md`.
...
- **RU:** `claude`: `<модель/alias>`, режим `<interactive|print-json>`, fallback `<альтернатива>`.
  **EN:** `claude`: `<model/alias>`, mode `<interactive|print-json>`, fallback `<alternative>`.
...
- **RU:** После чтения `local/ai/chat_context.md`/`local/ai/project_addenda.md` собрать открытые вопросы из `local/ai/session_summaries/`. Если требуется — использовать `<collect_open_items_script>`. Результат включить в первое сообщение.
...
- **RU:** При завершении работы формируем `local/ai/session_summaries/<ISO8601>.md` со структурой: «Контекст», «Системные указания», «Ключевые файлы» (`path:line + note`), «Последние события» (команда → результат), «Проблемы и решения», «Рекомендации / Следующие шаги» (чек-лист), «Дополнительные материалы», «Открытые вопросы».
  **EN:** When closing a session, **create** `local/ai/session_summaries/<ISO8601>.md` with the following sections: “Context”, “System directives”, “Key files” (`path:line` + note), “Recent events” (command → outcome), “Issues & fixes”, “Recommendations / Next steps” (checklist), “Additional materials”, and “Open questions.”
- **RU:** Временные каталоги CLI (`tmp/ai/gemini_home`, `tmp/ai/qwen_home`, `tmp/ai/copilot_home`, `tmp/ai/claude_home`, `tmp/ai/cli_tokens`) описываем в ответе и очищаем после подтверждения пользователя.
  **EN:** Document temporary CLI directories (`tmp/ai/gemini_home`, `tmp/ai/qwen_home`, `tmp/ai/copilot_home`, `tmp/ai/claude_home`, `tmp/ai/cli_tokens`) in replies and remove them once the user confirms.
