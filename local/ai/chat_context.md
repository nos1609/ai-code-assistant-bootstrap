...
- **RU:** CLI `gemini`, `qwen`, `codex`, `copilot`, `claude` доступны `<по умолчанию/после подтверждения>`.
...
- **RU:** Bootstrap-пункты (README комментарий, симлинки, `.git/info/exclude`, логи) `<проверены/ожидают проверки>`; отклонения фиксирую в `local/ai/session_history.md`.
