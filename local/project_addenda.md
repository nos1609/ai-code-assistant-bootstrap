...
- **RU:** Укажите разрешённые зоны записи (`<repo root>`, `tmp/`, `build/`) и запретите системные каталоги (`~/.<tool>`, `/etc`, `%APPDATA%`) без разрешения.  
  **EN:** Define allowed write locations (`<repo root>`, `tmp/`, `build/`) and forbid system paths (`~/.<tool>`, `/etc`, `%APPDATA%`) unless explicitly approved.
...
- **RU:** Разрешённые действия без подтверждения: чтение файлов, обновление `local/chat_context.md`, `local/session_history.md`, `local/session_summaries/`, восстановление симлинков, запуск локальных скриптов без сети.  
...
- **RU:** Требуют отдельного согласия: операции вне репозитория, сетевой доступ, запуск CLI ассистентов, изменение приватных конфигов, удаление файлов, правка `.gitignore`, переписывание истории Git.  
  **EN:** Require approval: actions outside the repo, network access, invoking assistant CLIs, editing private configs, deleting files, changing `.gitignore`, rewriting Git history.
- **RU:** Если токены лежат в `~/.<tool>`, согласуйте временный `tmp/cli_tokens/`, добавьте в `.gitignore`, опишите очистку.  
  **EN:** When tokens live in `~/.<tool>`, agree on a temporary `tmp/cli_tokens/`, add it to `.gitignore`, and document the cleanup plan.
...
- **RU:** При завершении работы формируем `local/session_summaries/<ISO8601>.md` со структурой: «Контекст», «Системные указания», «Ключевые файлы» (`path:line + note`), «Последние события» (команда → результат), «Проблемы и решения», «Рекомендации / Следующие шаги» (чек-лист), «Дополнительные материалы», «Открытые вопросы».  
  **EN:** When closing a session create `local/session_summaries/<ISO8601>.md` with “Context,” “System directives,” “Key files” (`path:line + note`), “Recent events” (command → outcome), “Issues & fixes,” “Recommendations / Next steps” (checklist), “Additional materials,” and “Open questions.”
- **RU:** Временные каталоги CLI (`tmp/gemini_home`, `tmp/qwen_home`, `tmp/copilot_home`, `tmp/cli_tokens`) описываем в ответе и очищаем после подтверждения пользователя.  
  **EN:** Document temporary CLI directories (`tmp/gemini_home`, `tmp/qwen_home`, `tmp/copilot_home`, `tmp/cli_tokens`) in replies and remove them once the user confirms.
...
- **RU:** Обновляйте этот файл не реже `<период>` и фиксируйте изменения в `local/session_history.md`.  
  **EN:** Review this file at least every `<cadence>` and log changes in `local/session_history.md`.
